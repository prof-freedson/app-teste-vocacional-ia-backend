...
export async function adminRoutes(app) {
    ...
    app.post("/admin/courses/import", {
        ...
        try {
            const { courses: importCourses, overwrite = false } = request.body;
            let existingCourses = overwrite ? [] : await loadCourses();
            const processedCourses = importCourses.map((course) => ({
                id: generateId(),
                nome: course.nome || "",
                area: course.area || "",
    ...
    app.put("/admin/config/whatsapp", {
        ...
        try {
            const { number, enabled = true } = request.body;
...
export { CourseSchema, ConfigSchema };
