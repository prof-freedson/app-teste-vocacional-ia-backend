...
export class QuestionAgent {
  ...
  async generateQuestion(
    userProfile: Partial<VocationalTestRequest>,
  ...
  private buildUserPrompt(
    ...
    if ((userProfile as any).categoria_foco) {
      prompt += `\n- Foco desta pergunta: ${(userProfile as any).categoria_foco}`;
    }
  ...
  private analyzeDominantAreas(answers: Record<string, any>): string[] {
  ...
  private analyzePersonalityTraits(answers: Record<string, any>): string[] {
    const traits: string[] = [];
