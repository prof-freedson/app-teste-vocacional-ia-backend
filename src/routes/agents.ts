...
async function loadCoursesFromAdmin(): Promise<Course[]> {
  try {
...
export async function agentRoutes(fastify: FastifyInstance) {
  ...
  fastify.post("/agents/questions", {
    ...
    try {
      const { questionCount = 10, userProfile, difficulty } = request.body as any;
      
      AgentLogger.logAgentCall('QuestionAgent', 'generateQuestionSet', Date.now());
      
      const questions = await questionAgent.generateQuestionSet(
        userProfile || {},
        questionCount
      );
  ...
  fastify.post("/agents/courses", {
    ...
    try {
      const { vocationalProfile, userRequest, area, maxCourses } = request.body as any;
  ...
  fastify.post("/agents/whatsapp", {
    ...
    try {
      const { 
        ...
        messageType = "resultado_completo" 
      } = request.body as any;
      ...
      switch (messageType) {
        case "resumo_rapido":
          ...
          message = await whatsAppAgent.formatQuickSummary(
            userRequest.nome,
            vocationalAnalysis.area_principal,
            courseRecommendations.curso_prioritario?.nome
          );
          break;
