...
async function saveCourses(courses: Course[]): Promise<void> {
  ...
  await fs.writeFile(
    COURSES_FILE_PATH,
    JSON.stringify(validatedCourses, null, 2),
    "utf-8"
  );
}
...
const ConfigSchema = z.object({
  ...
  senac: z
    .object({
      ...
      phone: z.string().default("(98) 31981530"),
      website: z.string().default("www.ma.senac.br"),
      email: z.string().default("cepsaoluis@ma.senac.br"),
      address: z
        .string()
        .default("Rua do Passeio, 495 - Centro, São Luís - MA"),
    })
    .optional(),
});
  ...
  try {
    ...
    const defaultConfig: Config = {
      ...
      senac: {
        ...
        email: "cepsaoluis@ma.senac.br",
        address: "Rua do Passeio, 495 - Centro, São Luís - MA",
      },
    };
...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        const courseData = request.body as Omit<
          Course,
  ...
  app.put(
    ...
    async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        const updateData = request.body as Omit<
          Course,
          "id" | "created_at" | "updated_at"
        >;
        const courses = await loadCourses();
  ...
  app.get("/admin/courses/export", async (request, reply) => {
    try {
      ...
      reply.header(
        "Content-Disposition",
        `attachment; filename="cursos_senac_${
    ...
    try {
      ...
      const stats = {
        ...
        por_nivel: courses.reduce((acc, course) => {
          acc[course.nivel || "basico"] =
            (acc[course.nivel || "basico"] || 0) + 1;
          return acc;
        }, {} as Record<string, number>),
        por_modalidade: courses.reduce((acc, course) => {
          acc[course.modalidade || "presencial"] =
            (acc[course.modalidade || "presencial"] || 0) + 1;
          return acc;
        }, {} as Record<string, number>),
      };
  ...
  app.put(
    ...
    async (request, reply) => {
      try {
        const { number, enabled = true } = request.body as {
          number: string;
          enabled?: boolean;
        };
        ...
        if (!isValidPhone && !isValidLink) {
          ...
          return {
            success: false,
            error:
              "Formato inválido. Use (XX) XXXXX-XXXX, (XX) XXXX-XXXX ou https://wa.me/559831981530",
          };
        }
